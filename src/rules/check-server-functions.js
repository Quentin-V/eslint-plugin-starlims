  ...
  create(context) {
    ...
    return {
      ...
      CallExpression(node) {
        ...
        if (object.name !== 'lims' || !linkFunctions.includes(property.name))
          return;
...
const getNonArrayAssignments = (variableName, scope) => {
  ...
  return references // Filter all the references that are not valid parameters, leave the ones that are array or too complex to check
    .filter((ref) => {
      ...
      // If the variable is a CallExpression, check if the object is an array
      if (writeExpr.type === 'CallExpression')
        return writeExpr.type === 'MemberExpression' // If it's a member expression, check if the object is an array
          ? writeExpr.object.type === 'ArrayExpression'
          : false; // Else don't check further
